...
\begin{ejercicioClase}
    \subsubsection{Conceptos de punteros y operadores}
    Responda justificando brevemente:
    \begin{enumerate}[label=\alph*)]
        \item ¿Qué diferencia hay entre los operadores unarios \texttt{\&} y \texttt{*}? Proporcione un ejemplo corto en C donde se utilicen ambos.
        \item ¿Cuál es el valor que representa un puntero nulo (NULL) y por qué es importante inicializar los punteros no utilizados con este valor?
        \item Si tenemos \texttt{int vec[5]; int *p = vec;}, ¿cuál es la diferencia entre \texttt{*(p + 3)} y \texttt{*p + 3}?
    \end{enumerate}

    \subsubsection{Seguimiento de código (Ruteo)}
    Determine la salida del siguiente programa y represente gráficamente (con cajas y flechas) el estado de las variables y punteros al final de la ejecución:
...
int main() {
    ...
    **dp = 35;
    ...
    \begin{enumerate}[a)]
        \item Implemente una función \texttt{void invertirCadena(char *cad)} que reciba una cadena de caracteres y la invierta en el lugar utilizando aritmética de punteros en lugar de índices de arreglos (no use corchetes \texttt{[]}).
        \item Escriba un programa que solicite un entero $N$. Reserve memoria dinámica utilizando \texttt{malloc} para almacenar $N$ números reales de tipo \texttt{double}. Luego, cargue los números, calcule su promedio y libere la memoria utilizando \texttt{free}.
    \end{enumerate}
\end{ejercicioClase}
