...
Definir una estructura llamada \texttt{registro_temperatura} con los miembros:

\begin{itemize}
\item día (entero)
\item mes (entero)
\item temperatura_max (flotante)
\item temperatura_min (flotante)
\end{itemize}
...
\begin{itemize}
\item \texttt{struct producto cargar_producto(void);}

Solicita al usuario los datos de un producto.

\item \texttt{void imprimir_producto(struct producto p);}
...
\begin{itemize}
\item \texttt{void cargar_producto(struct producto *p);}
Solicita al usuario los datos de un producto.
\item \texttt{void imprimir_producto(struct producto p);}
Muestra por pantalla los datos del producto.
\end{itemize}
...
\begin{itemize}
\item \texttt{void cargar_productos(struct producto *p, int tam);}

Solicita al usuario los datos de un producto.

\item \texttt{void imprimir_producto(struct producto *p, int tam);}
Muestra por pantalla los datos del producto.
\end{itemize}
...
\begin{itemize}
\item \texttt{void cargar_alumno(struct alumno *a);}
\item \texttt{void calcular_promedio(struct alumno *a);}
\item \texttt{void determinar_estado(struct alumno *a);}
→ “Promocionado” si promedio ≥ 8, “Regular” si ≥ 6, “Libre” si < 6.
\item \texttt{void imprimir_alumno(struct alumno a);}
\end{itemize}
...
\item \texttt{void cargar_producto(struct producto *p);}
\item \texttt{void imprimir_producto(struct producto p);}
\item \texttt{void actualizar_stock(struct producto *p, int cantidad);}
...
\item Llamar a \texttt{actualizar_stock}.
